...
export function needsUnicodeStripped(output: PandocOutput): boolean {
  return output.format === "latex" || output.format === "pdf" || output.format === "beamer";
}
...
export const pandoc = async (
  ...
  output: PandocOutput,
  extraParams?: string[],
): Promise<{ result: string; command: string; error: string }> =>
  new Promise(async (resolve, reject) => {
    ...
    if (output.format === "pdf" && (await lookpath("xelatex"))) args.push("--pdf-engine=xelatex");
    if (!stdin) {
